...
var remoteCreateCmd = &cobra.Command{
	Use:   "create",
	Short: "create thing on thingiverse",
	Long: `create thing on thingiverse, images and files, meta data, or all

Examples:
  thingiverse-cli remote create
  thingiverse-cli remote create --access_token=YOUR_ACCESS_TOKEN
  `,
...
func init() {
	remoteCreateCmd.Flags().String("access_token", "", "Access token for thingiverse")
	remoteCreateCmd.Flags().Bool("dry_run", false, "Preview operation")
	remoteCreateCmd.Flags().Bool("debug", false, "Show debug")
