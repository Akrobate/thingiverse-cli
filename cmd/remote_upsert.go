...
var remoteUpsertCmd = &cobra.Command{
	Use:   "upsert",
	Short: "upsert thing on thingiverse",
	Long: `upsert thing on thingiverse, images and files, meta data, or all

Examples:
  thingiverse-cli remote upsert
  thingiverse-cli remote upsert --access_token=YOUR_ACCESS_TOKEN
  `,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if updateType == "info" || updateType == "all" {
			t.Load()
			if t.Id == 0 {
				if err := t.Create(accessToken); err != nil {
					return fmt.Errorf("[ERROR] Create info: %w", err)
				}
			} else {
			...
			fmt.Println("[OK] Upsert info success")
		}
...
func init() {
	remoteUpsertCmd.Flags().String("access_token", "", "Access token for thingiverse")
	remoteUpsertCmd.Flags().Bool("dry_run", false, "Preview operation")
	remoteUpsertCmd.Flags().Bool("debug", false, "Show debug")
	remoteCmd.AddCommand(remoteUpsertCmd)
}
